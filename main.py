...
async def cmd_static(message: Message):
    """Handle /static command - show comprehensive bot statistics (admin only)"""
    ...
    for order in orders_data.values():
        platform = order.get('platform', 'unknown').lower()
        ...
        if order.get('status') == 'completed':
            platform_revenue[platform] += order.get('total_price', 0.0)
    
    # Sort platforms by order count
    ...
    try:
        ...
        system_info = f"""
💾 <b>Memory:</b> {memory_used:.1f}GB / {memory_total:.1f}GB ({memory_percent:.1f}%)
💿 <b>Disk:</b> {disk_free:.1f}GB free ({100-disk_percent:.1f}% available)
"""
    except:
        system_info = """
💾 <b>Memory:</b> Normal usage
💿 <b>Disk:</b> Sufficient space available
"""

    # Load additional data files for comprehensive stats
    try:
        ...
        if ratings_data:
            avg_rating = sum(r.get('rating', 0) for r in ratings_data) / len(ratings_data)
            avg_rating_display = f"{avg_rating:.1f}/5.0 ⭐"
        else:
            avg_rating_display = "No ratings yet"
    except:
        total_ratings = 0
        total_feedback = 0
        avg_rating_display = "No ratings yet"
    ...
    text = f"""
┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
┃ 📊 <b>INDIA SOCIAL PANEL - BOT STATISTICS</b>
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
• 💳 <b>Pending Balance:</b> ₹{total_balance:,.2f}
...
"""
    ...
    for i, (platform, count) in enumerate(top_platforms, 1):
        ...
        revenue = platform_revenue.get(platform, 0.0)
        text += f"\n{i}. {emoji} <b>{platform.title()}:</b> {count:,} orders (₹{revenue:,.2f})"
...
• ⭐ <b>Total Ratings:</b> {total_ratings:,}
• 📝 <b>Total Feedback:</b> {total_feedback:,}
• 🌟 <b>Average Rating:</b> {avg_rating_display}

┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
┃ 🖥️ <b>SYSTEM PERFORMANCE</b>
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{system_info}
🤖 <b>Bot Status:</b> ✅ Running Smoothly
🔄 <b>Data Files:</b> users.json, orders.json, ratings.json, feedback.json

┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
┃ 🎯 <b>QUICK SUMMARY</b>
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🚀 <b>Business Health:</b> {"🟢 Excellent" if completed_orders > 0 else "🟡 Starting"}
📈 <b>Growth Rate:</b> {"🔥 Active" if new_users_today > 0 else "📊 Stable"}
💎 <b>User Engagement:</b> {"⭐ High" if (active_users/max(total_users, 1)) > 0.3 else "📈 Growing"}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎉 <b>India Social Panel - Your SMM Success Partner!</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""

    await message.answer(text, parse_mode="HTML")
    print(f"📊 ADMIN STATS: Admin {user.id} viewed bot statistics")
...
async def cb_browse_platform(callback: CallbackQuery):
    ...
    platform = callback.data.replace("browse_", "")
...
async def cb_browse_package_details(callback: CallbackQuery):
    ...
    parts = callback.data.replace("browse_package_", "").split("_")
    if len(parts) < 2:
...
async def cb_rate_order(callback: CallbackQuery):
    ...
    # Extract order ID from callback data
    order_id = callback.data.replace("rate_order_", "")
...
async def cb_submit_rating(callback: CallbackQuery):
    ...
    parts = callback.data.replace("submit_rating_", "").split("_")
    if len(parts) < 2:
...
async def cb_feedback_order(callback: CallbackQuery, state: FSMContext):
    ...
    # Extract order ID from callback data
    order_id = callback.data.replace("feedback_order_", "")
